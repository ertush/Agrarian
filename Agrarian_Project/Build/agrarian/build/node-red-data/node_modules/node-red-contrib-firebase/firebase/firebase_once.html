        ...
        oneditprepare: function() { //TODO: there is all kinds of validation that needs to be done to ensure only proper combinations of things are allowed...
          ...
          var operators = [
            ...
            $('#eventType-select').on('change', function(){
              if ($(this).val() == "shallow_query"){ //disable/hide query paramters window and button
                ...
                $('#eventType-select-info-container').fadeOut(300, function(){
                  ...
                  //Check to see if ok button should be enabled
            ...
            function checkForIllegalComnbinationOfQueries(select, operator){
              ...
              if(operatorOptions[operator] && operatorOptions[operator].cannotBeCombinedWith){
                for (var i in operatorOptions[operator].cannotBeCombinedWith){
            ...
            function removeInputFromUI($input){
              ...
              $input.parent().hide()  // hide the input and the typedInput div
              //Check all other inputs and see whether or not they have text
              if (checkInputsNotEmpty() && checkSelectsValid()) $('#node-dialog-ok').prop('disabled', false);
              else $('#node-dialog-ok').prop('disabled', true);
            }
            ...
            function generatequery(query){
              ...
              for (var d in operators){
                ...
                //Check whether operator is used AND whether it's valid
                var used = false;
                $('#node-input-query-container select').each(function(idx){
              ...
              var $tdInput = $('<td/>', {style:"width:200px;"}).appendTo($tbody);
              ...
              else $select.prop('selectedIndex', -1); //I still have this here, but it doesn't actually work -- the element must be added to the UI first before this works (stupid...), so it is done below after it is appended to the table
              var currentOperator = $select.val();

              //Check if input is allowed
              if (currentOperator && operatorOptions[currentOperator] && operatorOptions[currentOperator].inputAllowed) addInputToUI($input);
            ...
            $("#node-input-add-query").click(function(e) {
              console.log(e)
              e.preventDefault();
              var queries = $('#node-input-query-container').children();
